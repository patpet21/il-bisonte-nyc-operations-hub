(function(){
  ...
  function openEditor(row=null){
    ...
    form.addEventListener('submit',async e=>{e.preventDefault();const data=Object.fromEntries(new FormData(form).entries());data.hours=data.hours===''?'':number(data.hours);data.rate=data.rate===''?'':number(data.rate);data.amount=data.amount===''?0:number(data.amount);try{const saved=await persistEntry(row?.entryId||'',data);const savedId=row?.entryId||saved?.entryId||'';if(savedId)await syncOnsiteFee(savedId,data);close();await refreshRows(true);if(typeof App!=='undefined'&&App.page==='dashboard'&&typeof render==='function')render();if(typeof toast==='function')toast(row?'Work entry updated':'Work entry logged');}catch(err){if(typeof toast==='function')toast(err.message);else alert(err.message);}});
  }
