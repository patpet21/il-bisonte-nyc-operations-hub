(function(){
  ...
  function canEdit(){const s=window.IBAuth?.current?.();if(s?.permissions&&typeof s.permissions.manageProjects==='boolean')return s.permissions.manageProjects;return ['project_manager','it_admin'].includes(App.role)}
  function parseCustom(value){if(Array.isArray(value))return value;try{const x=JSON.parse(value||'[]');return Array.isArray(x)?x:[]}catch(e){return []}}

  function renderWorkspace(p){
    const shell=q('.pw-shell');if(!shell)return;const editable=canEdit(),tasks=(App.data.tasks||[]).filter(t=>t.projectId===p.id),custom=parseCustom(p.customSections);
    shell.innerHTML=`<header class="pw-head"><div class="pw-head-main"><div class="pw-project-id">${safe(p.id)} · ${safe(p.scope||'Project')}</div><h2 class="pw-title" id="pwTitle">${safe(p.name)}</h2><div class="pw-subline"><span class="pill ${cls(p.priority)}">${safe(p.priority||'Normal')}</span><span class="pill ${cls(p.status)}">${safe(p.status||'Not Started')}</span>${editable?'':'<span class="pw-view-only">View only</span>'}</div></div><div class="pw-head-actions"><span class="pw-unsaved" id="pwUnsaved"></span>${editable?'<button class="pw-save" id="pwSave">Save changes</button>':''}<button class="pw-close" id="pwClose" aria-label="Close">×</button></div></header>
      <nav class="pw-tabs"><button class="pw-tab active" data-pw-tab="overview">Overview</button><button class="pw-tab" data-pw-tab="details">Details</button><button class="pw-tab" data-pw-tab="tasks">Tasks <span class="muted">${tasks.length}</span></button><button class="pw-tab" data-pw-tab="notes">Notes & Links</button><button class="pw-tab" data-pw-tab="custom">Custom Sections <span class="muted">${custom.length}</span></button><button class="pw-tab" data-pw-tab="activity">Activity</button></nav>
      <div class="pw-body"><section class="pw-panel active" data-pw-panel="overview">${overviewPanel(p,editable)}</section><section class="pw-panel" data-pw-panel="details">${detailsPanel(p,editable)}</section><section class="pw-panel" data-pw-panel="tasks">${tasksPanel(tasks,editable)}</section><section class="pw-panel" data-pw-panel="notes">${notesPanel(p,editable)}</section><section class="pw-panel" data-pw-panel="custom">${customPanel(custom,editable)}</section><section class="pw-panel" data-pw-panel="activity">${activityPanel(p)}</section></div>`;
    wire(p,editable);
  }

  function overviewPanel(p,edit){return `<div class="pw-section"><div class="pw-section-head"><div><h3>Project Overview</h3><p>The essential information anyone should understand immediately.</p></div></div><div class="pw-grid pw-grid-3">${field('Project name','name',p.name,'text',edit,'full')}${field('Owner','owner',p.owner,'text',edit)}${field('Scope / Area','scope',p.scope,'text',edit)}${selectField('Priority','priority',p.priority,['Low','Medium','High','Urgent'],edit)}${selectField('Status','status',p.status,['Not Started','In Progress','In Review','Blocked','Completed','On Hold'],edit)}${field('Target date','due',p.due,'date',edit)}<div class="pw-field full"><span>Progress</span>${edit?`<div class="pw-progress-row"><input class="pw-range" id="pwProgress" name="progress" type="range" min="0" max="100" value="${num(p.progress)}"><input class="pw-input" id="pwProgressNumber" type="number" min="0" max="100" value="${num(p.progress)}"></div>`:`<div class="pw-readonly">${num(p.progress)}%</div>`}</div>${area('Next action','nextAction',p.nextAction,edit,'full')}</div></div>`}
  function detailsPanel(p,edit){return `<div class="pw-section"><div class="pw-section-head"><div><h3>Purpose & Context</h3><p>Why the project exists and what success means.</p></div></div><div class="pw-grid">${area('Description','description',p.description,edit,'full tall')}${area('Objective','objective',p.objective,edit,'full')}</div></div><div class="pw-section"><div class="pw-section-head"><div><h3>People & Delivery</h3><p>Who is involved and what must be produced.</p></div></div><div class="pw-grid">${area('Stakeholders','stakeholders',p.stakeholders,edit,'full')}${area('Deliverables','deliverables',p.deliverables,edit,'full')}</div></div><div class="pw-section"><div class="pw-section-head"><div><h3>Risks & Dependencies</h3><p>Keep blockers and dependencies visible before they become surprises.</p></div></div><div class="pw-grid">${area('Risks','risks',p.risks,edit,'full')}${area('Dependencies','dependencies',p.dependencies,edit,'full')}</div></div>`}
  ...
  function customPanel(custom,edit){return `<div class="pw-note">Create any extra section you need without turning the Hub into a complicated enterprise tool: meeting notes, budget, purchase list, technical details, store feedback, approvals, lessons learned, or anything else.</div><div class="pw-section"><div class="pw-section-head"><div><h3>Custom Sections</h3><p>Unlimited flexible sections inside this project.</p></div>${edit?'<button class="pw-add-task" id="pwAddSection">+ Add section</button>':''}</div><div id="pwCustomList">${custom.length?custom.map((s,i)=>customCard(s,i,edit)).join(''):'<div class="pw-task-empty" id="pwCustomEmpty">No custom sections yet.</div>'}</div></div>`}
  function customCard(s,i,edit){return `<div class="pw-section pw-custom-section" data-custom-index="${i}" style="margin-bottom:10px"><div class="pw-section-head"><div style="flex:1">${edit?`<input class="pw-input" data-custom-title value="${safe(s.title||'Untitled section')}" placeholder="Section title">`:`<h3>${safe(s.title||'Untitled section')}</h3>`}</div>${edit?'<button class="pw-task-delete" data-custom-delete title="Remove section">×</button>':''}</div>${edit?`<textarea class="pw-textarea tall" data-custom-body placeholder="Add anything you need here…">${safe(s.body||'')}</textarea>`:`<div class="pw-readonly">${safe(s.body||'—').replace(/\n/g,'<br>')}</div>`}</div>`}
  ...
  function wire(p,edit){
    q('#pwClose').onclick=attemptClose;q('#pwBackdrop').onclick=e=>{if(e.target===e.currentTarget)attemptClose()};qa('.pw-tab').forEach(b=>b.onclick=()=>switchTab(b.dataset.pwTab));if(!edit)return;
    q('#pwSave').onclick=save;qa('.pw-input,.pw-select,.pw-textarea,.pw-range').forEach(el=>el.addEventListener('input',markDirty));
    ...
    q('#pwAddTask')?.addEventListener('click',()=>{q('#pwTaskEmpty')?.remove();const row=document.createElement('div');row.innerHTML=taskRow({id:`NEW-${Date.now()}`,title:'',owner:'',priority:'Medium',status:'Not Started',due:''},true);const node=row.firstElementChild;q('#pwTaskList').appendChild(node);wireTask(node);markDirty();node.querySelector('.pw-task-title')?.focus()});qa('.pw-task').forEach(wireTask);
    q('#pwAddSection')?.addEventListener('click',()=>{q('#pwCustomEmpty')?.remove();const wrap=document.createElement('div');wrap.innerHTML=customCard({title:'New Section',body:''},Date.now(),true);const node=wrap.firstElementChild;q('#pwCustomList').appendChild(node);wireCustom(node);markDirty();q('[data-custom-title]',node)?.select()});qa('.pw-custom-section').forEach(wireCustom);
  }
  function wireTask(row){qa('input,select',row).forEach(el=>el.addEventListener('input',markDirty));q('.pw-task-check',row)?.addEventListener('change',e=>{const status=q('[data-task-field="status"]',row);if(status)status.value=e.target.checked?'Completed':'In Progress';markDirty()});q('.pw-task-delete',row)?.addEventListener('click',()=>{const id=row.dataset.taskId;if(row.dataset.new!=='1')deletedTasks.add(id);row.remove();markDirty();if(!q('.pw-task',q('#pwTaskList')))q('#pwTaskList').innerHTML='<div class="pw-task-empty" id="pwTaskEmpty">No tasks yet. Add the first action for this project.</div>'})}
  function wireCustom(node){qa('input,textarea',node).forEach(el=>el.addEventListener('input',markDirty));q('[data-custom-delete]',node)?.addEventListener('click',()=>{node.remove();markDirty();if(!q('.pw-custom-section',q('#pwCustomList')))q('#pwCustomList').innerHTML='<div class="pw-task-empty" id="pwCustomEmpty">No custom sections yet.</div>'})}
  ...
  async function save(){
    ...
    try{
      const patch={};qa('[name]',q('.pw-shell')).forEach(el=>patch[el.name]=el.value);patch.progress=q('#pwProgressNumber')?.value||project.progress||'0';patch.customSections=JSON.stringify(qa('.pw-custom-section').map(node=>({title:q('[data-custom-title]',node)?.value||'Untitled section',body:q('[data-custom-body]',node)?.value||''})));
