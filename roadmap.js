(function(){
  ...
  const fallbackPhases=[
    {id:'PH-00',name:'Emergency Stabilization & Continuity',status:'Partially Completed',period:'Aug–Sep 2026',objective:'Restore basic store connectivity and keep operations running while the permanent network path is corrected.',nextAction:'Close remaining stabilization actions into Network Stabilization.'},
    {id:'PH-01',name:'Network, Firewall & Wi‑Fi Stabilization',status:'In Progress',period:'Sep 2026',objective:'Establish a documented WatchGuard-centered network baseline with stable DHCP, segmentation, VPN and upstairs Wi‑Fi coverage.',nextAction:'Validate final firewall/segmentation state and schedule upstairs AP assessment.'},
    {id:'PH-02',name:'Retail Pro Prism & Remote Access',status:'In Progress',period:'Sep 2026',objective:'Stabilize Retail Pro Prism connectivity and document the DNS/VPN/remote-support path.',nextAction:'Italy IT to validate Prism name resolution and VPN dependency.'},
    {id:'PH-03',name:'Documentation, Assets, Vendors & Credentials',status:'In Progress',period:'Sep–Oct 2026',objective:'Create a reliable operational baseline for systems, hardware, vendors, credentials, SOPs and ownership.',nextAction:'Finish asset baseline and confirm credential owners/recovery paths.'},
    {id:'PH-04',name:'NYC Operations Hub & Access Governance',status:'In Progress',period:'Sep 2026',objective:'Provide one private role-based workspace for requests, projects, vendors, procedures, users, Pass and management visibility.',nextAction:'Configure Google Web OAuth client and deploy Apps Script backend.'},
    {id:'PH-05',name:'Retail & Store Process Mapping',status:'Planned',period:'Sep–Oct 2026',objective:'Map inventory, reporting and recurring store/Italy workflows before automating anything.',nextAction:'Interview Damiano and Store Manager; document current workflow.'},
    {id:'PH-06',name:'Automation Node, UPS, Monitoring & Reporting',status:'Planned',period:'Oct 2026',objective:'Create a resilient local automation node for approved workflows and operational monitoring.',nextAction:'Define mini-PC workload and UPS requirements before purchase.'},
    {id:'PH-07',name:'Physical Security & Continuous Improvement',status:'Planned',period:'Later / Pilot',objective:'Close remaining physical-security items and run the Hub as an ongoing improvement system.',nextAction:'Schedule physical security review after infrastructure stabilization.'}
  ];
  const milestones=[
  ...
  function init(){
    ['project_manager','management'].forEach(role=>{const list=App.nav[role];if(!list||list.some(x=>x[0]==='roadmap'))return;const i=list.findIndex(x=>x[0]==='dashboard');list.splice(i>=0?i+1:0,0,['roadmap','◆','Roadmap'])});
    const previous=renderPage;
    renderPage=function(){if(App.page==='roadmap'){renderRoadmap();return;}previous()};
    renderNav();if(typeof render==='function')render();
  }
  function phaseRows(){return window.IBRoadmapPhaseStore?.getAll?.()||fallbackPhases}
  function renderRoadmap(){
    const root=q('#pageRoot');if(!root)return;
    const projects=App.data.projects||[],requests=App.data.requests||[],phases=phaseRows();
    const active=projects.filter(p=>p.status==='In Progress').length;
    const planned=projects.filter(p=>p.status==='Not Started').length;
    const high=requests.filter(r=>r.priority==='High'&&!['Resolved','Closed'].includes(r.status)).length;
    root.innerHTML=`${pageHead('NYC Transformation Roadmap','A reconstructed baseline of what has already happened, what is still open, and what comes next.','PROGRAM BASELINE')}<div class="roadmap-shell">
      ...
      <section class="roadmap-section"><div class="roadmap-section-head"><div><h3>Program Phases</h3><p>Click any phase to open and edit it.</p></div></div><div class="phase-grid">${phases.map(phaseCard).join('')}</div></section>
      ...
      <section class="roadmap-section"><div class="roadmap-section-head"><div><h3>Project Portfolio</h3><p>Click a card to open, edit, add tasks, notes, links and details.</p></div><button class="btn" data-go="projects">Open Projects</button></div><div class="roadmap-projects">${projects.map(projectCard).join('')}</div></section>
    ...
    qa('[data-go]',root).forEach(b=>b.onclick=()=>{App.page=b.dataset.go;render()});
    qa('[data-roadmap-project]',root).forEach(c=>c.onclick=()=>window.IBProjectWorkspace?.open(c.dataset.roadmapProject));
  }
  function phaseCard(p){const c=slug(p.status);return `<article class="phase-card ${c}" data-phase-id="${safe(p.id)}" tabindex="0" role="button" aria-label="Open roadmap phase ${safe(p.name)}"><div class="phase-top"><span class="phase-id">${safe(p.id)} · ${safe(p.period)}</span><span class="phase-status ${c}">${safe(p.status)}</span></div><h4>${safe(p.name)}</h4><p>${safe(p.objective)}</p><div class="phase-next"><span>Next action · Open & edit</span><strong>${safe(p.nextAction||p.next||'To be defined')}</strong></div></article>`}
