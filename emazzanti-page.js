(function(){
  ...
  function renderEmazzanti(root){
    ...
    root.innerHTML=head()+`
      <section class="emz-hero"><div><div class="emz-kicker">CURRENT STATUS</div><h2>${escapeHtml(offer.status)}</h2><p>${escapeHtml(offer.objective)}</p><div class="emz-hero-actions"><a class="btn" href="${MEETING_URL}" target="_blank" rel="noopener noreferrer">Open meeting reference ↗</a><button class="btn" id="emzOpenProject">Open linked project</button></div></div><div class="emz-summary"><div><span>Meeting</span><strong>${escapeHtml(offer.meetingDate)}</strong></div><div><span>Project</span><strong>PRJ-0012</strong></div><div><span>Vendor status</span><strong>${escapeHtml(vendor?.status||'Assessment / Proposal')}</strong></div><div><span>Transcript</span><strong>Available</strong></div></div></section>
      ...
      <section class="panel emz-section"><div class="panel-head"><div><div class="emz-kicker">PROPOSED SERVICE MODEL</div><h3>What eMazzanti says it can provide</h3></div><span class="emz-source">Confirmed from meeting recap</span></div>${cards(offer.capabilities)}</section>
      ...
      <section class="panel emz-section"><div class="panel-head"><div><div class="emz-kicker">OPEN ACTIONS</div><h3>Assessment & proposal workstream</h3></div><span class="emz-source">Linked to PRJ-0012</span></div>${actionRows()}${project?`<div class="emz-next"><strong>Current project next action</strong><span>${escapeHtml(project.nextAction||'')}</span></div>`:''}</section>
      <section class="panel emz-section emz-commercial"><div class="panel-head"><div><div class="emz-kicker">COMMERCIAL / CONTRACTUAL</div><h3>Still pending before approval</h3></div><span class="emz-warning">Not yet supplied</span></div>${cards(offer.pending)}<div class="emz-callout"><strong>Important</strong><span>The meeting recap describes capabilities and next steps, but it is not yet a complete priced managed-services proposal. These items should remain open until eMazzanti supplies the formal commercial documentation.</span></div></section>
      <section class="panel emz-section"><div class="panel-head"><div><div class="emz-kicker">KEY CONTACTS</div><h3>eMazzanti coordination</h3></div></div>${contacts()}<div class="emz-note">Nirvan remains the commercial coordination point. Pietro is the local operational contact for the technical follow-up, with Damiano representing Italy-side IT requirements and decisions.</div></section>
