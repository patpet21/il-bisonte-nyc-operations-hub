...
const LOGIN_TOUCH_TTL_SECONDS=600;
let IB_SPREADSHEET_CACHE=null;
...
function doPost(e){
  try{
    ...
    let data;
    if(action==='bootstrap') data=bootstrapSession_(identity);
    else if(action==='getSession') data=getSession_(identity);
...
function spreadsheet_(){
  if(IB_SPREADSHEET_CACHE)return IB_SPREADSHEET_CACHE;
  const id=PropertiesService.getScriptProperties().getProperty('SPREADSHEET_ID');
  if(!id)throw new Error('Missing Script Property SPREADSHEET_ID');
  IB_SPREADSHEET_CACHE=SpreadsheetApp.openById(id);
  return IB_SPREADSHEET_CACHE;
}
function authMode_(){return String(PropertiesService.getScriptProperties().getProperty('AUTH_MODE')||'google').toLowerCase();}
function firebaseApiKey_(){return PropertiesService.getScriptProperties().getProperty('FIREBASE_API_KEY')||FIREBASE_WEB_API_KEY_FALLBACK;}
function firebaseProjectId_(){return PropertiesService.getScriptProperties().getProperty('FIREBASE_PROJECT_ID')||FIREBASE_PROJECT_ID_FALLBACK;}

function authenticateIdentity_(idToken){
  if(authMode_()==='demo')return {provider:'demo',email:'demo.pm@ilbisonte.local',name:'Demo Project Manager',sub:'demo',emailVerified:true};
  if(!idToken)throw new Error('Authentication required');
  const cached=readCachedIdentity_(idToken);if(cached)return cached;
  const payload=decodeJwtPayload_(idToken),issuer=String(payload.iss||'');
  const identity=issuer.indexOf('https://securetoken.google.com/')===0?verifyFirebaseIdentity_(idToken,payload):verifyGoogleIdentity_(idToken);
  enforceAllowedDomain_(identity.email);cacheIdentity_(idToken,identity);return identity;
}
function verifyGoogleIdentity_(idToken){
  ...
  return {provider:'google',email:email,name:info.name||email,sub:info.sub||'',picture:info.picture||'',emailVerified:true};
}
function verifyFirebaseIdentity_(idToken,payload){
  const projectId=firebaseProjectId_(),apiKey=firebaseApiKey_();
  if(!projectId||!apiKey)throw new Error('Firebase Authentication backend is not configured');
  if(String(payload.aud||'')!==projectId)throw new Error('Firebase token audience mismatch');
  if(String(payload.iss||'')!=='https://securetoken.google.com/'+projectId)throw new Error('Firebase token issuer mismatch');
  if(Number(payload.exp||0)<=Math.floor(Date.now()/1000))throw new Error('Firebase session expired');
  const response=UrlFetchApp.fetch('https://identitytoolkit.googleapis.com/v1/accounts:lookup?key='+encodeURIComponent(apiKey),{method:'post',contentType:'application/json',payload:JSON.stringify({idToken:idToken}),muteHttpExceptions:true});
  if(response.getResponseCode()!==200)throw new Error('Invalid or expired email/password sign-in');
  const info=JSON.parse(response.getContentText()||'{}'),user=(info.users||[])[0];
  if(!user||user.disabled)throw new Error('Email/password account is unavailable');
  const email=String(user.email||payload.email||'').trim().toLowerCase();if(!email)throw new Error('Firebase account email unavailable');
  return {provider:'firebase',email:email,name:user.displayName||email,sub:user.localId||payload.user_id||'',picture:user.photoUrl||'',emailVerified:Boolean(user.emailVerified)};
}
function decodeJwtPayload_(token){try{const parts=String(token||'').split('.');if(parts.length<2)return {};return JSON.parse(Utilities.newBlob(Utilities.base64DecodeWebSafe(parts[1])).getDataAsString())}catch(e){return {}}}
function identityCacheKey_(token){const digest=Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,String(token),Utilities.Charset.UTF_8);return 'auth:'+digest.slice(0,16).map(b=>('0'+((b+256)%256).toString(16)).slice(-2)).join('')}
function readCachedIdentity_(token){try{const raw=CacheService.getScriptCache().get(identityCacheKey_(token));return raw?JSON.parse(raw):null}catch(e){return null}}
...
function touchUserLogin_(identity,user){
  const cache=CacheService.getScriptCache(),key='login-touch:'+String(identity.email||'').toLowerCase();
  try{if(cache.get(key))return user}catch(e){}
  const sh=spreadsheet_().getSheetByName(SHEETS.users);if(!sh)return user;const values=sh.getDataRange().getValues();if(values.length<2)return user;
  const headers=values[0],map=headerMap_(headers);const idx=values.findIndex((r,i)=>i>0&&String(r[map.email]||'').trim().toLowerCase()===identity.email);if(idx<1)return user;
  if(map.lastLogin!==undefined)values[idx][map.lastLogin]=date_('yyyy-MM-dd HH:mm');if(identity.provider==='google'&&map.googleSub!==undefined)values[idx][map.googleSub]=identity.sub||'';
  sh.getRange(idx+1,1,1,headers.length).setValues([values[idx]]);try{cache.put(key,'1',LOGIN_TOUCH_TTL_SECONDS)}catch(e){}
...
function readObjects_(sheetName){
  ...
  const sh=spreadsheet_().getSheetByName(sheetName);if(!sh)return [];const values=sh.getDataRange().getDisplayValues();if(values.length<2)return [];
  const headers=values[0],rows=values.slice(1).filter(r=>r.some(Boolean)).map(row=>normalizeObject_(objectFromRow_(headers,row)));
  if(isOperationalCacheSheet_(sheetName)){try{const json=JSON.stringify(rows);if(json.length<90000)cache.put(key,json,READ_CACHE_TTL_SECONDS)}catch(e){}}
  return rows;
}
function invalidateSheetCache_(sheetName){try{CacheService.getScriptCache().remove('sheet:'+sheetName+':v1')}catch(e){}}
...
function updateProject_(id,patch,user){const editable=['name','scope','owner','priority','status','due','nextAction','description','objective','stakeholders','deliverables','risks','progress','dependencies','notes','links','attachments','customSections'];const updated=updateRowById_(SHEETS.projects,id,patch,editable,user,'Project updated','PROJECT');logActivity_('Project workspace saved','PROJECT',id,`${id} · ${updated.name||'Project'} updated`,user.displayName||user.email||'Project Manager');return updated;}
function createTask_(payload,user){const sh=spreadsheet_().getSheetByName(SHEETS.tasks);if(!sh)throw new Error('TASKS sheet not found');const id=nextId_(sh,'TSK');const headers=sh.getRange(1,1,1,sh.getLastColumn()).getDisplayValues()[0];const obj={id:id,projectId:payload.projectId||'',title:payload.title||'New task',owner:payload.owner||'',status:payload.status||'Not Started',priority:payload.priority||'Medium',dueDate:payload.due||payload.dueDate||'',notes:payload.notes||''};sh.appendRow(headers.map(h=>obj[toCamel_(h)]!==undefined?obj[toCamel_(h)] : ''));invalidateSheetCache_(SHEETS.tasks);logActivity_('Task created','TASK',id,`${obj.projectId} · ${id} created: ${obj.title}`,user.displayName||user.email||'Project Manager');return normalizeObject_(obj);}
...
function deleteTask_(id,user){const sh=spreadsheet_().getSheetByName(SHEETS.tasks);if(!sh)throw new Error('TASKS sheet not found');const values=sh.getDataRange().getValues();const idx=values.findIndex((r,i)=>i>0&&String(r[0])===String(id));if(idx<1)return {ok:true};const projectId=values[idx][1]||'';sh.deleteRow(idx+1);invalidateSheetCache_(SHEETS.tasks);logActivity_('Task removed','TASK',id,`${projectId} · ${id} removed`,user.displayName||user.email||'Project Manager');return {ok:true};}
function updateRowById_(sheetName,id,patch,editable,user,action,entityType){if(!id)throw new Error('Missing id');const sh=spreadsheet_().getSheetByName(sheetName);if(!sh)throw new Error(sheetName+' sheet not found');const values=sh.getDataRange().getValues(),headers=values[0],map=headerMap_(headers);const idx=values.findIndex((r,i)=>i>0&&String(r[0])===String(id));if(idx<1)throw new Error('Record not found: '+id);editable.forEach(key=>{if(patch[key]===undefined)return;const mapped=key==='due'?'dueDate':key;if(map[mapped]!==undefined)values[idx][map[mapped]]=patch[key]});if(map.updatedAt!==undefined)values[idx][map.updatedAt]=date_('yyyy-MM-dd');sh.getRange(idx+1,1,1,headers.length).setValues([values[idx]]);invalidateSheetCache_(sheetName);const actor=user.displayName||user.email||'Project Manager';logActivity_(action,entityType,id,`${id} updated`,actor);return normalizeObject_(objectFromRow_(headers,values[idx]));}

function requestAccess_(payload,identity){const email=identity.email,sh=spreadsheet_().getSheetByName(SHEETS.users);if(!sh)throw new Error('USERS sheet not found');const role=validateRole_(payload.role||'store_manager'),values=sh.getDataRange().getValues(),headers=values[0],map=headerMap_(headers),idx=values.findIndex((r,i)=>i>0&&String(r[map.email]||'').toLowerCase()===email);const rowObj={email:email,displayName:payload.displayName||identity.name||email,role:role,status:'Pending',requestedAt:date_('yyyy-MM-dd HH:mm'),approvedBy:'',approvedAt:'',store:payload.store||'New York',pushSubscription:'',lastLogin:date_('yyyy-MM-dd HH:mm'),googleSub:identity.provider==='google'?(identity.sub||''):'',department:payload.department||'',notes:'',accessVersion:'1'};if(idx>0){Object.keys(rowObj).forEach(k=>{if(map[k]!==undefined)values[idx][map[k]]=rowObj[k]});sh.getRange(idx+1,1,1,headers.length).setValues([values[idx]])}else sh.appendRow(headers.map(h=>rowObj[toCamel_(h)]||''));logActivity_('Access requested','USER',email,`${rowObj.displayName} requested ${rowObj.role} access`,rowObj.displayName);createNotification_({recipient:'role:project_manager',title:'New access request',message:`${rowObj.displayName} requested ${prettyRole_(rowObj.role)} access.`,type:'access_request',entityType:'USER',entityId:email,sendEmail:false});createNotification_({recipient:'role:management',title:'Approval required',message:`${rowObj.displayName} is waiting for access approval.`,type:'approval',entityType:'USER',entityId:email,sendEmail:false});sendAdminEmail_('Il Bisonte Operations — new access request',`${rowObj.displayName} (${email}) requested ${prettyRole_(rowObj.role)} access for ${rowObj.store}.`);return rowObj;}
...
function logActivity_(action,entityType,entityId,details,actor){const sh=spreadsheet_().getSheetByName(SHEETS.activity);if(!sh)return;sh.appendRow([nextId_(sh,'ACT'),date_('yyyy-MM-dd HH:mm'),actor||'System',action,entityType,entityId,details||'']);invalidateSheetCache_(SHEETS.activity);}
